...
async def _process_pd_organisation(
    ...
    pd_org_id
    """
    with logfire.span('process_pd_organisation'):
        ...
        if company:
            if current_pd_org:
                ...
                old_company_cf_vals = (
                    await old_pd_org.custom_field_values(company_custom_fields) if old_org_data else {}
                )
        ...
        # Commented out to stop the infinite loop, need to change to only be triggered if inherited custom fields are changed
...
async def _process_pd_person(current_pd_person: Optional[Person], old_pd_person: Optional[Person]) -> Contact | None:
    ...
    pd_person_id
    """
    with logfire.span('process_pd_person'):
...
async def _process_pd_deal(current_pd_deal: Optional[PDDeal], old_pd_deal: Optional[PDDeal]) -> Deal | None:
    ...
    with logfire.span('process_pd_deal'):
...
async def _process_pd_pipeline(
    ...
    """
    with logfire.span('process_pd_pipeline'):
...
async def _process_pd_stage(current_pd_stage: Optional[PDStage], old_pd_stage: Optional[PDStage]) -> Stage | None:
    ...
    updates it if it did
    """
    with logfire.span('process_pd_stage'):
