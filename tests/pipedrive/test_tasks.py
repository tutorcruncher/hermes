...
class TestContactToPersonData:
    """Test _contact_to_person_data conversion function"""

    def test_contact_with_email_and_phone(self, db, test_contact):
        """Test contact with both email and phone number returns correct data"""
        test_contact.email = 'john@example.com'
        test_contact.phone = '+44 20 1234 5678'
        db.add(test_contact)
        db.commit()

        result = _contact_to_person_data(test_contact, db)

        assert result['name'] == test_contact.name
        assert result['emails'] == [{'value': 'john@example.com', 'label': 'work', 'primary': True}]
        assert result['phones'] == [{'value': '+44 20 1234 5678', 'label': 'work', 'primary': True}]
        assert 'org_id' in result
        assert 'owner_id' in result

    def test_contact_with_email_but_no_phone(self, db, test_contact):
        """Test contact with email but no phone uses placeholder phone number"""
        test_contact.email = 'jane@example.com'
        test_contact.phone = None
        db.add(test_contact)
        db.commit()
        ...
        assert result['name'] == test_contact.name
        assert result['emails'] == [{'value': 'jane@example.com', 'label': 'work', 'primary': True}]
        # Should use placeholder phone when phone is None
        assert result['phones'] == [{'value': '077777777777', 'label': 'work', 'primary': True}]
        assert 'org_id' in result
        assert 'owner_id' in result

    def test_contact_with_email_but_empty_phone(self, db, test_contact):
        """Test contact with email but empty string phone uses placeholder phone number"""
        test_contact.email = 'bob@example.com'
        test_contact.phone = ''
        db.add(test_contact)
        db.commit()
        ...
        assert result['name'] == test_contact.name
        assert result['emails'] == [{'value': 'bob@example.com', 'label': 'work', 'primary': True}]
        # Should use placeholder phone when phone is empty string
        assert result['phones'] == [{'value': '077777777777', 'label': 'work', 'primary': True}]

    def test_contact_with_phone_but_no_email(self, db, test_contact):
        """Test contact with phone but no email succeeds (assertion passes)"""
        test_contact.email = None
        test_contact.phone = '+1 555 123 4567'
        db.add(test_contact)
        db.commit()
        ...
        assert result['name'] == test_contact.name
        assert result['phones'] == [{'value': '+1 555 123 4567', 'label': 'work', 'primary': True}]
        # Email will be None
        assert result['emails'] == [{'value': None, 'label': 'work', 'primary': True}]

    def test_contact_with_phone_but_empty_email(self, db, test_contact):
        """Test contact with phone but empty string email succeeds (assertion passes with empty string)"""
        test_contact.email = ''
        test_contact.phone = '+1 555 987 6543'
        db.add(test_contact)
        db.commit()

        # Empty string is falsy, so assertion should fail
        with pytest.raises(AssertionError):
            _contact_to_person_data(test_contact, db)

    def test_contact_with_neither_email_nor_phone(self, db, test_contact):
        """Test contact with neither email nor phone raises AssertionError"""
        test_contact.email = None
        test_contact.phone = None
        db.add(test_contact)
        db.commit()
    ...
    def test_contact_with_empty_email_and_empty_phone(self, db, test_contact):
        """Test contact with empty string email and phone raises AssertionError"""
        test_contact.email = ''
        test_contact.phone = ''
        db.add(test_contact)
        db.commit()
    ...
    def test_contact_includes_org_id_from_company(self, db, test_contact):
        """Test that contact data includes org_id from associated company"""
        test_contact.company.pd_org_id = 99999
        db.add(test_contact.company)
        db.commit()

        result = _contact_to_person_data(test_contact, db)

        assert result['org_id'] == 99999

    def test_contact_includes_owner_id_from_company_sales_person(self, db, test_contact):
        """Test that contact data includes owner_id from company's sales person"""
        test_contact.company.sales_person.pd_owner_id = 88888
        db.add(test_contact.company.sales_person)
        db.commit()

        result = _contact_to_person_data(test_contact, db)

        assert result['owner_id'] == 88888

    def test_contact_with_invalid_phone_characters_uses_placeholder(self, db, test_contact):
        """Test contact with invalid phone (e.g., 'N/A' or 'unknown') still works if email exists"""
        test_contact.email = 'user@example.com'
        test_contact.phone = 'N/A'  # Invalid phone number
        db.add(test_contact)
        db.commit()

        result = _contact_to_person_data(test_contact, db)

        # Should use the actual phone value, not the placeholder
        # The placeholder is only used when phone is None or empty
        assert result['phones'] == [{'value': 'N/A', 'label': 'work', 'primary': True}]
